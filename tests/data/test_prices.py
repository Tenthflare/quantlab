...
def test_history_never_returns_the_future(store):
    end_date = pd.Timestamp("2020-01-06")
...
def test_history_boundary_on_a_non_trading_day(store):
    ...
    window = store.history_lookback(pd.Timestamp("2020-01-04"), lookback=252)
...
def test_history_lookback_counts_trading_days(store):
    ...
    window = store.history_lookback(pd.Timestamp("2020-01-07"), lookback=2)
...
def test_price_at_returns_cross_section(store):
    p = store.price_at(pd.Timestamp("2020-01-06"), "close_adj")
