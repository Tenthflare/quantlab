...
class BacktestResult:
    ...
    def equity_curve(self) -> pd.Series:
        return (1.0 + self.records["PnL"]).cumprod()
...
class BacktestEngine:
    ...
    def run(self, rebalance_dates, universe) -> BacktestResult:
        ...
        for t, t_next in zip(dates[:-1], dates[1:]):
            ...
            fwd_return = self.prices.calculate_realized_return(t, t_next, universe)    # earned AFTER t
            ...
            rows.append({"date": t, "gross_pnl": gross_return, "cost": cost,
                         "pnl": gross_return - cost, "turnover": float(trades.abs().sum())})
